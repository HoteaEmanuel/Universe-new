...
import { useOpportunitiesFilters } from "./hooks/useOpportunitiesFilters";
...
export default function OpportunitiesPage() {
  ...
  const { filters, setFilters } = useOpportunitiesFilters();
  const { opportunityType, workplaceType, status, sort, savedOnly } = filters;
  ...
  const queryFilters = useMemo<OpportunityFilters>(
    () => ({
      q: trimmedQuery,
      opportunityType,
      workplaceType,
    ...
    [trimmedQuery, opportunityType, workplaceType, status, sort, savedOnly],
  );
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (
        remaining < SCROLL_FETCH_THRESHOLD &&
        query.hasNextPage &&
      ...
      <header className="flex flex-col gap-4 rounded-2xl bg-primary/[0.07] p-5 dark:bg-brand-400/10 sm:p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-balance text-2xl font-bold tracking-tight">
              Jobs and internships for student life
            </h1>
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              Discover opportunities from verified organizations, save the
              promising ones, and apply on the employer’s website.
            </p>
          </div>
        </div>
        <div className="relative">
          ...
          <Input
            ...
            placeholder="Search roles, companies, skills, or majors"
        ...
        <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
          <Select
            ...
            onValueChange={(value: unknown) =>
              setFilters({
                opportunityType:
                  value === ALL ? undefined : (value as OpportunityType),
              })
            }
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="graduate_program">
                Graduate programs
              </SelectItem>
              <SelectItem value="volunteering">Volunteering</SelectItem>
              <SelectItem value="campus_ambassador">
                Campus ambassador
              </SelectItem>
            </SelectContent>
          </Select>
          <Select
            value={workplaceType ?? ALL}
            onValueChange={(value: unknown) =>
              setFilters({
                workplaceType:
                  value === ALL ? undefined : (value as WorkplaceType),
              })
            }
          >
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="expired">Closed</SelectItem>
          ...
          <Select
            value={sort}
            onValueChange={(value: unknown) =>
              setFilters({ sort: value as typeof sort })
            }
          >
          ...
          <Button
            ...
            onClick={() => setFilters({ savedOnly: !savedOnly })}
          >
            Saved only
          </Button>
        </div>
      ...
      <div ref={listRef} className="flex flex-col gap-6">
        ...
        {!query.isPending && !query.isError && posts.length === 0 && (
          <div className="rounded-2xl border border-dashed border-border py-14 text-center">
            ...
            <p className="mt-3 font-semibold">No matching opportunities</p>
            <p className="mt-1 text-sm text-muted-foreground">
